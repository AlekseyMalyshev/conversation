app.controller('UsersCtrl', ['$scope', '$http', '$state',
  function($scope, $http, $state) {
    ...
    $scope.message = function() {
      ...
      $http.post('/api/conversations', conversation).then(function(response) {
        $state.go('conversation', {conversationId: response.data._id});
      }, function(err) {
        var text;
        if (err.status === 409) {
